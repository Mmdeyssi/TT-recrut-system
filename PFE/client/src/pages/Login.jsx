    ...
    <div className='flex items-center justify-center min-h-screen px-6 sm:px-0 bg-gradient-to-br from-blue-200 to-purple-400'>
        ...
        <form onSubmit={onSubmitHandler}>
            {state === 'Sign Up' && (
                <>  <div className='mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C]'>
                          ...
                          <input onChange={e => setFullname (e.target.value)} value={fullName} className='bg-transparent outline-none' type="text" placeholder="fullName" required />
                    </div>   
                    <div className='mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C]'>
                              <img src={assets.age} alt="" />
                              <input onChange={e => setAge (e.target.value)} value={age}  className='bg-transparent outline-none' type="text" placeholder="Age" required />
                    </div>          
                    <div className='mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C]'>
                              <img src={assets.phone} alt="" />
                              <input onChange={e => setPhone (e.target.value)} value={phone} className='bg-transparent outline-none' type="text" placeholder="Phone" required />
                    </div>
                     </>
       ...
       >
         ...
         <option value="employer">Employer</option>
          ...
          <button className='w-full py-2.5 mt-5 rounded-full bg-gradient-to-r from-indigo-500 to-indigo-900 text-white font-medium'>{state === 'Sign Up' ? 'Sign Up' : 'Login' } </button>
        </form>
        {state === 'Sign Up' ? (
